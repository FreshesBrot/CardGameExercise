...
struct Command {
	...
	Command(bool arbitrary = false) : name(), paramCount(0), optCount(0), parameters(paramCount), options(optCount), b_allowArbitrary(arbitrary) { };
	...
	Command(Command&& c) noexcept {
		...
		parameters = Tokens(std::move(c.parameters));
		options = Tokens(std::move(c.options));

		b_allowArbitrary = c.b_allowArbitrary;
		c.b_allowArbitrary = false;
	};
	...
	Tokens parameters; //all parameters
	Tokens options; //all options
...
public:
	...
	inline const Tokens& getTokens() const { return tokens; };
	...
	static bool containsElement(Tokens& vect, const Token& elem) {
		for (auto& check : vect)
			if (check == elem) return true;
		return false;
	}
...
private:
	...
	Tokens tokens; //tokenized command string
